...
export function generateQuotationPDF(quotation, res) {
  ...
  /* ================= COLORS ================= */
  const PRIMARY_COLOR = "#1a365d"; // Professional blue
  const ACCENT_COLOR = "#2d3748"; // Dark gray
  const HIGHLIGHT_COLOR = "#2b6cb0"; // Blue for highlights
  const BORDER_COLOR = "#e2e8f0"; // Light gray for borders

  /* ================= PAGE FRAME ================= */
  const PAGE_BOTTOM = 750; // HARD LIMIT
  let y = 40;
 ...
 try {
  ...
  if (quotation.person_name) {
    doc
      .font("Helvetica-Oblique") // Italic font
  ...
  // Total with highlight - FIXED: Save total amount to variable
  ...
  /*doc
    .fontSize(8)
    .text(
      `© ${new Date().getFullYear()} Bab Alyusr Business Setup Services. All rights reserved.`,
      40,
      footerY + 10,
      { width: 515, align: "center" }
    );*/
