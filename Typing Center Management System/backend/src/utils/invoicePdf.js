...
export function generateInvoicePDF(invoice, res) {
  ...
  doc.fontSize(9); // GLOBAL SMALLER TEXT
  ...
  doc.fillColor("#2d3748").font("Helvetica").fontSize(9); // smaller card text
  ...
  if (invoice.payments && invoice.payments.length > 0) {
    ...
    // Cash payments
    const cashTotal = invoice.payments
    ...
    // Bank transfers
    const bankTotal = invoice.payments
  ...
  const amountForWords = invoice.balance > 0 && invoice.payments?.length > 0
    ? invoice.balance
    : invoice.total;
  ...
  if (y < PAGE_BOTTOM - 100) {
    doc
      .rect(40, y, 515, 50) // Reduced height
      .fillColor("#f8fafc")
      .fill()
      .strokeColor(BORDER_COLOR)
      .stroke();
  ...
  const thanksY = Math.max(y + 50 + 10, 600); // minimum space
  ...
  const noteY = Math.max(thanksY + 50, 680); // Position for system note
  ...
  /* ================= FOOTER ================= */
  // Footer separator - moved up to be after system note
   const footerY = 770;
